...
const aquariumRouter = express.Router();
...
const validateAquariumInput = (req, res, next) => {
...
aquariumRouter.post(
  "/submit",
  ejsAuthenticate,
  upload.array('aquariumPhotos', 5),
  validatePhotos,
  validateAquariumInput,
  async (req, res) => {
    try {
      const imageUrls = await uploadMultipleImagesToSupabase(req.files, 'aquarium');
      const { sellerName, contactNumber, email, pincode, city, state, auctionType, basePrice, auctionDuration, itemTitle, desription } = req.body;
      ...
      const aquariumItem = new Aquarium({ sellerName, contactNumber, email, pincode, city, state, auctionType, basePrice, auctionDuration, itemTitle, desription, photo1: imageUrls[0], photo2: imageUrls[1], photo3: imageUrls[2], photo4: imageUrls[3], photo5: imageUrls[4], username });
  aquariumItem.userid = req.user && req.user._id;
  await aquariumItem.save();
  const result = { message: 'Aquarium item submitted successfully!', itemId: aquariumItem._id };
  return sendResultOrRedirect(req, res, result, '/');
      ...
      console.error('Error submitting Aquarium item:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  }
);

aquariumRouter.get("/", async (req, res) => {
  try {
    const items = await Aquarium.find().sort({ createdAt: -1 });
    res.json(items);
  } catch (error) {
    console.error('Error fetching Aquarium items:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

aquariumRouter.get("/:id", async (req, res) => {
  try {
    const item = await Aquarium.findById(req.params.id);
    if (!item) return res.status(404).json({ error: 'Item not found' });
    res.json(item);
  } catch (error) {
    console.error('Error fetching Aquarium item:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

aquariumRouter.use((error, req, res, next) => {
