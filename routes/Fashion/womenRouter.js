...
const validateWomenInput = (req, res, next) => {
...
womenRouter.post(
  "/submit",
  ejsAuthenticate,
  upload.array('womenPhotos', 5),
  validatePhotos,
  validateWomenInput,
  async (req, res) => {
    try {
      const imageUrls = await uploadMultipleImagesToSupabase(req.files, 'women');
      const { sellerName, contactNumber, email, pincode, city, state, auctionType, basePrice, auctionDuration, itemTitle, desription } = req.body;
      ...
      const womenItem = new Women({ sellerName, contactNumber, email, pincode, city, state, auctionType, basePrice, auctionDuration, itemTitle, desription, photo1: imageUrls[0], photo2: imageUrls[1], photo3: imageUrls[2], photo4: imageUrls[3], photo5: imageUrls[4], username });
  womenItem.userid = req.user && req.user._id;
  await womenItem.save();
  const result = { message: 'Women fashion item submitted successfully!', itemId: womenItem._id };
  return sendResultOrRedirect(req, res, result, '/');
      ...
      console.error('Error submitting Women fashion item:', error);
...
womenRouter.get("/", async (req, res) => {
  try {
    const items = await Women.find().sort({ createdAt: -1 });
    res.json(items);
  } catch (error) {
    console.error('Error fetching Women fashion items:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
womenRouter.get("/:id", async (req, res) => {
  try {
    ...
    console.error('Error fetching Women fashion item:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

womenRouter.use((error, req, res, next) => {
