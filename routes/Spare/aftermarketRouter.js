...
const aftermarketRouter = express.Router();
...
const validateAftermarketInput = (req, res, next) => {
...
aftermarketRouter.post(
  "/submit",
  ejsAuthenticate,
  upload.array('aftermarketPhotos', 5),
  validatePhotos,
  validateAftermarketInput,
  async (req, res) => {
    try {
      const imageUrls = await uploadMultipleImagesToSupabase(req.files, 'aftermarket');
      const { sellerName, contactNumber, email, pincode, city, state, auctionType, basePrice, auctionDuration, itemTitle, description } = req.body;
      ...
      const aftermarketItem = new Aftermarket({ sellerName, contactNumber, email, pincode, city, state, auctionType, basePrice, auctionDuration, itemTitle, description, photo1: imageUrls[0], photo2: imageUrls[1], photo3: imageUrls[2], photo4: imageUrls[3], photo5: imageUrls[4], username });
  aftermarketItem.userid = req.user && req.user._id;
  await aftermarketItem.save();
  const result = { message: 'Aftermarket item submitted successfully!', itemId: aftermarketItem._id };
  return sendResultOrRedirect(req, res, result, '/');
      ...
      console.error('Error submitting Aftermarket item:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  }
);

aftermarketRouter.get("/", async (req, res) => {
  try {
    const items = await Aftermarket.find().sort({ createdAt: -1 });
    res.json(items);
  } catch (error) {
    console.error('Error fetching Aftermarket items:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

aftermarketRouter.get("/:id", async (req, res) => {
  try {
    const item = await Aftermarket.findById(req.params.id);
    if (!item) return res.status(404).json({ error: 'Item not found' });
    res.json(item);
  } catch (error) {
    console.error('Error fetching Aftermarket item:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

aftermarketRouter.use((error, req, res, next) => {
...
module.exports = aftermarketRouter;
