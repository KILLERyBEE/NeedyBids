...
async function setPincode() {
  ...
  try {
    ...
    if (data[0].Status === "Success") {
      const district = data[0].PostOffice[0].Name;
...
function useCurrentLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      // Use a more reliable geocoding service with proper error handling
      ...
      try {
        ...
        if (data.error) {
          showError(`Error from location service: ${data.error}`);
          return;
        }
        
        // Extract location details more reliably
        const address = data.address;
        const district = address.city || address.town || address.village || 
                         address.county || address.state_district || "Unknown District";
        const pincode = address.postcode || "";
        
        if (district === "Unknown District") {
          showError("Could not determine your location. Please try entering your pincode manually.");
          return;
        }
        ...
        showError("Failed to fetch location details. Please try again or enter your pincode manually.");
      }
    },
    (error) => {
      ...
      switch (error.code) {
        case error.PERMISSION_DENIED:
          showError("Location access denied. Please allow location access or enter your pincode manually.");
          break;
        case error.POSITION_UNAVAILABLE:
          showError("Location information is unavailable. Please check your connection or enter your pincode manually.");
          break;
        case error.TIMEOUT:
          showError("Location request timed out. Please try again or enter your pincode manually.");
          break;
        default:
